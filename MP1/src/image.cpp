...
namespace img {
    ...
    void image::setColor( const pixel & px ) {
        for( unsigned int i = 0; i < pixels.size(); ++i){ pixels[i] = px; }
    }
    
    void image::insertSubImage( int x, int y, const image & simg ) {
        int mx = std::min(x + simg.width(), w);
        int my = std::min(y + simg.height(), h);
        
        for(unsigned int i = x; i < mx; ++i){
            for(unsigned int j = y; j < my; ++j){
                (*this)(i,j) = simg(i-x,j-y);
            }
        }
    }
